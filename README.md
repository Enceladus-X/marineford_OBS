...
1. GitHub Releases에서 `Marineford_OBS_v1.1.exe`를 다운로드합니다.
2. 송출컴의 원하는 폴더에 EXE를 둡니다.
3. EXE를 더블클릭합니다.
4. 자동으로 열리는 송출용 패널에서 태블릿 연결 QR을 스캔합니다.

EXE 옆에는 실행 중 `state.json`, `events.jsonl`, `deck_presets.json`, `images/`가 생성될 수 있습니다. 현장 상태, 덱 프리셋, 업로드 이미지 저장용 파일이므로 정상 동작입니다.
...
1. 송출컴에서 `Marineford_OBS_v1.1.exe` 또는 `start_marineford.bat`을 실행합니다.
...
- `control.html`: 송출용 패널, 태블릿 QR, 방송 설정, 덱 이미지 프리셋 관리
...
빌드 결과는 `dist/Marineford_OBS_v1.1.exe`에 생성됩니다.
