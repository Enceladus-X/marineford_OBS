...
1. GitHub Releases에서 `Marineford_OBS_v1.5.exe`를 다운로드합니다.
...
덱 이미지를 업로드할 때는 자르기 창의 `실제 송출 영역` 틀을 기준으로 위치, 확대, 좌우반전을 조절합니다. 원본과 송출용 crop은 별도 파일로 보존되므로 다시 편집해도 화질과 구도를 복구할 수 있습니다.
송출용 패널에서 게임별 룰 프리셋을 저장하고 목록에서 다시 불러올 수 있습니다. 기본값은 BO3, 기본 타이머 40분, 엑스트라 턴 15분입니다.
컨트롤 패널 상단에서 송출 화면, 태블릿 화면, 편집 로그를 바로 열 수 있습니다.
...
1. 송출컴에서 `Marineford_OBS_v1.5.exe` 또는 `start_marineford.bat`을 실행합니다.
2. 송출용 패널에서 대회명, 선수 닉네임, 덱 이미지, OBS용 표시 요소를 세팅합니다.
...
- `images/`: 소스 ZIP의 기본 이미지 에셋 및 실행 중 생성되는 로컬 업로드 이미지
...
빌드 결과는 `dist/Marineford_OBS_v1.5.exe`에 생성됩니다.
