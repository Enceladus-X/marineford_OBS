...
1. GitHub Releases에서 `Marineford_OBS_v1.4.exe`를 다운로드합니다.
...
덱 이미지를 업로드할 때는 자르기 창에서 위치, 확대, 좌우반전을 조절한 뒤 사용할 이미지를 확정합니다. 이미지 1은 세로형 슬롯에 꽉 차게 들어가도록 자르는 것을 기준으로 합니다.
...
1. 송출컴에서 `Marineford_OBS_v1.4.exe` 또는 `start_marineford.bat`을 실행합니다.
...
빌드 결과는 `dist/Marineford_OBS_v1.4.exe`에 생성됩니다.
